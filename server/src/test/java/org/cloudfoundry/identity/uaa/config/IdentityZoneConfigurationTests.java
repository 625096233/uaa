...
public class IdentityZoneConfigurationTests {
    ...
    public void testDefaultCorsConfiguration() {
        assertEquals(Arrays.asList(new String[] {ACCEPT, AUTHORIZATION, CONTENT_TYPE}), definition.getDefaultConfiguration().getAllowedHeaders());
        assertEquals(Arrays.asList(GET.toString()), definition.getDefaultConfiguration().getAllowedMethods());
        assertEquals(Arrays.asList(".*"), definition.getDefaultConfiguration().getAllowedUris());
        assertEquals(Collections.EMPTY_LIST, definition.getDefaultConfiguration().getAllowedUriPatterns());
        assertEquals(Arrays.asList(".*"), definition.getDefaultConfiguration().getAllowedOrigins());
        assertEquals(Collections.EMPTY_LIST, definition.getDefaultConfiguration().getAllowedOriginPatterns());
        assertEquals(1728000, definition.getDefaultConfiguration().getMaxAge());
    }
    ...
    public void testDeserialize_DefaultCorsConfiguration() {
        ...
        assertEquals(Arrays.asList(new String[] {ACCEPT}), definition.getDefaultConfiguration().getAllowedHeaders());
        assertEquals(Arrays.asList(new String[] {GET.toString(), POST.toString()}), definition.getDefaultConfiguration().getAllowedMethods());
        assertEquals(Arrays.asList(new String[] {"^/uaa/userinfo$", "^/uaa/logout\\.do$"}), definition.getDefaultConfiguration().getAllowedUris());
        assertEquals(Collections.EMPTY_LIST, definition.getDefaultConfiguration().getAllowedUriPatterns());
        assertEquals(Arrays.asList(new String[] {"^localhost$", "^.*\\.localhost$"}), definition.getDefaultConfiguration().getAllowedOrigins());
        assertEquals(Collections.EMPTY_LIST, definition.getDefaultConfiguration().getAllowedOriginPatterns());
        assertEquals(1728000, definition.getDefaultConfiguration().getMaxAge());
    }
}
